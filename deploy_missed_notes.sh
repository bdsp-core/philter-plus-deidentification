#!/bin/bash
#
# DEPLOYMENT for Missed Clinical Notes De-identification
#
# Instances are created MANUALLY by the user. This script:
#   1. Splits 200 missed clinical notes files evenly across N instances by file index range
...
#
# Input:  s3://bdsp-site-mgb/I0001_ClinicalNotes_Missed/  (200 files, ~544K rows each, ~108.8M total)
# Output: s3://bdsp-site-mgb/philter-deidentify/missed_notes_output/partition_{N}/
#
# Usage:
#   ./deploy_missed_notes.sh <instance-id-1> <instance-id-2> ... <instance-id-N>
#
...
TOTAL_FILES=200                           # Known total: 200 parquet files, ~544K rows each, ~108.8M records
NOTE_TYPE="clinicalnotes"                 # Uses NoteTXT + NoteCSNID columns
SSH_USER="ec2-user"
...
if [ $# -eq 0 ]; then
    echo "Usage: ./deploy_missed_notes.sh <instance-id-1> <instance-id-2> ... <instance-id-N>"
    echo ""
    ...
    echo "  ./deploy_missed_notes.sh i-0abc123 i-0def456 ..."
    exit 1
fi
...
echo "=========================================="
echo "AWS Philter Missed Clinical Notes Deployment"
echo "=========================================="
echo ""
echo "Configuration:"
...
echo "  Note type:    $NOTE_TYPE (NoteTXT + NoteCSNID)"
...
for i in $(seq 0 $((NUM_INSTANCES - 1))); do
    ...
    aws ec2 --profile $AWS_PROFILE create-tags \
        ...
        --tags "Key=Name,Value=Philter-MissedNotes-${i}" \
               "Key=Partition,Value=${i}" \
               "Key=Project,Value=Philter-MissedNotes" || true
...
for i in $(seq 0 $((NUM_INSTANCES - 1))); do
    ...
    cat > /tmp/missednotes_setup_${i}.sh <<SETUP_SCRIPT
#!/bin/bash
set -e
exec > >(tee /home/ec2-user/philter-setup.log) 2>&1

echo "=========================================="
echo "Philter Missed Notes Setup - Partition $i"
echo "Started: \$(date)"
echo "=========================================="
...
echo "=========================================="
echo "Watchdog MissedNotes - Partition ${i}"
echo "Input:    s3://${BUCKET}/${INPUT_PREFIX}"
...
SETUP_SCRIPT
    ...
    scp -o StrictHostKeyChecking=no -o ConnectTimeout=30 -o ServerAliveInterval=60 \
        -i "$KEY_FILE" /tmp/missednotes_setup_${i}.sh $SSH_USER@${IP}:/tmp/setup.sh
...
echo "         --stats-file   ./missed_notes_stats.csv \\"
echo "         --note-type    $NOTE_TYPE"
echo ""
echo "=========================================="
echo ${INSTANCE_IDS[@]} > /tmp/philter-missednotes-instance-ids.txt
echo "Instance IDs saved to: /tmp/philter-missednotes-instance-ids.txt"
