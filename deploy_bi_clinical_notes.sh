#!/bin/bash
#
# DEPLOYMENT for BI Clinical Notes De-identification
#
# Instances are created MANUALLY by the user. This script:
#   1. Assigns 1 file per instance (20 files, 20 instances)
...
#
# Input:  s3://bdsp-site-mgb/bi_clinical_notes/  (20 files, ~8.67M rows each, ~197.8M total)
# Output: s3://bdsp-site-mgb/philter-deidentify/bi_clinical_notes_output/partition_{N}/
#
# Usage:
#   ./deploy_bi_clinical_notes.sh <instance-id-1> <instance-id-2> ... <instance-id-N>
#
...
TOTAL_FILES=20                            # 20 parquet files, ~8.67M rows each, ~197.8M total
NOTE_TYPE="bi_clinicalnotes"              # Uses TEXT + CLINICALNOTETEXTKEY + DeidentifiedName columns
SSH_USER="ec2-user"
...
if [ $# -eq 0 ]; then
    echo "Usage: ./deploy_bi_clinical_notes.sh <instance-id-1> <instance-id-2> ... <instance-id-N>"
    echo ""
    ...
    echo ""
    echo "Example (20 instances):"
    echo "  ./deploy_bi_clinical_notes.sh i-0abc123 i-0def456 ..."
    exit 1
fi
...
# 1 file per instance (TOTAL_FILES must equal NUM_INSTANCES for clean split)
FILES_PER=1

echo "=========================================="
echo "AWS Philter BI Clinical Notes Deployment"
echo "=========================================="
echo ""
echo "Configuration:"
...
echo "  Note type:    $NOTE_TYPE (TEXT + CLINICALNOTETEXTKEY + DeidentifiedName)"
...
for i in $(seq 0 $((NUM_INSTANCES - 1))); do
    ...
    aws ec2 --profile $AWS_PROFILE create-tags \
        ...
        --tags "Key=Name,Value=Philter-BIClinicalNotes-${i}" \
               "Key=Partition,Value=${i}" \
               "Key=Project,Value=Philter-BIClinicalNotes" || true
...
for i in $(seq 0 $((NUM_INSTANCES - 1))); do
    ...
    cat > /tmp/bi_clinicalnotes_setup_${i}.sh <<SETUP_SCRIPT
#!/bin/bash
set -e
exec > >(tee /home/ec2-user/philter-setup.log) 2>&1

echo "=========================================="
echo "Philter BI Clinical Notes Setup - Partition $i"
echo "Started: \$(date)"
echo "=========================================="
...
# Remove any expired SSO credentials so IAM role is used automatically
...
echo "=========================================="
echo "Watchdog BIClinicalNotes - Partition ${i}"
echo "Input:    s3://${BUCKET}/${INPUT_PREFIX}"
...
while true; do
    ...
    if [ \$EXIT_CODE -eq 0 ]; then
        ...
        # Upload status CSV
        aws s3 cp /home/ec2-user/status_partition_${i}.csv \
            s3://${BUCKET}/${OUTPUT_PREFIX}/logs/status_partition_${i}.csv 2>/dev/null || true
        break
    fi
...
# ---- Start watchdog (setsid — survives SSH disconnect) ----
echo ""
echo "Starting watchdog (background, persistent)..."
setsid bash /home/ec2-user/start_watchdog.sh > /home/ec2-user/deidentify.log 2>&1 &
WATCHDOG_PID=\$!
...
SETUP_SCRIPT
    ...
    scp -o StrictHostKeyChecking=no -o ConnectTimeout=30 -o ServerAliveInterval=60 \
        -i "$KEY_FILE" /tmp/bi_clinicalnotes_setup_${i}.sh $SSH_USER@${IP}:/tmp/setup.sh
...
echo "         --stats-file   ./bi_clinical_notes_stats.csv \\"
echo "         --note-type    $NOTE_TYPE"
echo ""
echo "=========================================="
echo ${INSTANCE_IDS[@]} > /tmp/philter-bi-clinicalnotes-instance-ids.txt
echo "Instance IDs saved to: /tmp/philter-bi-clinicalnotes-instance-ids.txt"
