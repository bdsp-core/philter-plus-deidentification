...
3) After you have the environment set up correctly please make sure you have the notes that you need to deidentify in .txt format and place it in the input folder. 
4) The deidentified output file will have the same name as the input file so make sure that you do not have any PHI in the name of the file.
5) For the purpose of this code we are using data/input as the input directory to store all the identified notes which need to be deidentified.
6) Please run the following command to deidentify your notes: 
